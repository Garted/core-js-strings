...
function formatTime(minutes, seconds) {
  return String(Math.abs(minutes))
    .padStart(2, '0')
    .concat(':', String(Math.abs(seconds)).padStart(2, '0'));
}
...
function isPalindrome(str) {
  const mutStr = str.replaceAll(/[\s.,!?]/g, '').toLowerCase();
  return mutStr === mutStr.split('').reverse().join('');
}
...
function encodeToRot13(str) {
  ...
  const rod13 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let returnValue = '';

  for (let i = 0; i < str.length; i += 1) {
    if (alp.includes(str[i])) {
      returnValue += rod13[alp.indexOf(str[i])];
    } else {
      returnValue += str[i];
    }
  }
